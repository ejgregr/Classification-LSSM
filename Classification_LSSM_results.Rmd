...
## Variable selection
We first examined the seasonal predictors for cross-correlations, and Where the correlation between two predictors was > 0.6, we removed one. Removal was based on the presumed relevance of each predictor to the seasonal biology of bull kelp.  

For summer, we prioritized monthly surface temperatures, preferentially retaining the mean followed by the maximum as waters are generally still cool (i.e., unlikely to have reached critical temps REF). For salinity, we prioritized bottom salinity, also preferring the mean followed by the maximum to emphasize areas of well mixed, nutrient-rich ocean waters. We retained current predictors only if they were correlated at less than 0.6 with temperature or salinity, again preferentially retaining the mean followed by the maximum to emphasize areas of mixing. For winter, given the importance of gametophyte settlement and transport, we prioritized mean bottom currents, temperature, and salinity, preferring means followed by minimum values.  
...
We applied a systematic process to identify and remove the most cross-correlated predictors. 
...
- If you don’t need a caption, omit `fig.cap` and the plot is treated as inline content.  
-->
...
plot(sk1, col = 1:nclust, border=NA, main = "Summer clusters" ) 
plot(sk2, col = 1:nclust, border=NA, main = "Winter clusters" )
...
## Violin plots
```{r Violplots, results='asis', echo=FALSE, fig.cap="Violin plots of predictors across the July and December k-means clusters.", fig.width=7, fig.height=8, fig.align='center', out.extra='keepaspectratio', fig.pos='H'}
( jul_viols / dec_viols )
